...
class ProductGroup(QGroupBox):
    def __init__(self):
        ...
        self.color_type_line = QComboBox(form)
        self.color_type_line.addItems(COLOR_TYPES)
        ...
        form_layout.addRow(TABLE_HEADERS[3], self.color_type_line)
    ...
    def get_form_values(self):
        ...
        color_type = self.color_type_line.currentText().strip()
        num = self.num_line.text().strip()
        return width, height, color, color_type, num

    def form_is_valid(self):
        width, height, color, color_type, num = self.get_form_values()
    ...
    def get_data(self) -> dict:
        ...
        for row in range(self.table.rowCount()):
            ...
            color_type = self.table.item(row, 3)
            ...
            row_values['color_type'] = color_type.text() if color_type else ''
            row_values['num'] = num.text() if num else ''
            values.append(row_values)
        return {'table': values}

    def set_data(self, data: dict): 
        pass # TODO: write set_data for ProductGroup
